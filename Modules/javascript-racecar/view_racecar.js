...
const mcl_transport = dgram.createSocket({
  type: 'udp4',
  reuseAddr: true
});
...
mcl_transport.on('message', (msg, rinfo) => {
  const t = Date.now();
  // console.log(`server got message from ${rinfo.address}:${rinfo.port}`, msg.length);
  ...
  switch (msg.readUInt16BE(2)) {
    case 0x3032: // Decode LCM single: 0x4c43 3032
      ...
      const ch = msg.slice(8, ch_off0);
      const payload = msg.slice(ch_off0 + 1);
      const msg_mp = Buffer.concat([
        Buffer.from([0x81, 0xd9, ch.length]),
        ch, payload]);
    ...
    case 0x3033: // Decode LCM multiple: 0x4c43 3033
      ...
      if (frag_id === 0) {
        ...
        const ch = msg.slice(offset, ch_off);
        if (ch.length > 255) { ch = ch.slice(0, 255); }
        frag = Buffer.concat([
          new Buffer.from([0x81, 0xd9, ch.length]),
          ch,
          msg.slice(ch_off + 1)]);
      } else {
