...
lcm_hdr_t = ffi.typeof"struct lcm_hdr_t"
lcm_hdr_sz = ffi.sizeof"struct lcm_hdr_t"
lcm_hdr_ptr = ffi.typeof"struct lcm_hdr_t *"
...
local function playback_multiple(lognames, callbacks)
  ...
  for _, logname in ipairs(lognames) do
    ...
    local ok = coresume(co_play, logname)
  ...
  while #states > 0 do
    ...
    local co_min = s[1]
    local ok, str, ch, t_us, cnt = coresume(co_min)
