...
lib.to_xyz = to_xyz
local function packed_xyz(distances, azimuths, pts, cur)
  pts = pts or {}
  cur = cur or (#pts + 1)
...
lib.packed_xyz = packed_xyz

local function column_intensities(intensities, is, cur)
  is = is or {}
  cur = cur or (#is + 1)
  for j, int0 in ipairs(intensities) do
    for i, int in ipairs(int0) do
      is[cur] = int
      cur = cur + 1
    end
  end
...
lib.column_intensities = column_intensities

local function column_xyz(distances, azimuths)
  local xs, ys, zs = {}, {}, {}
  for j, azi in ipairs(azimuths) do
    for i, d in ipairs(distances[j]) do
      local p = polar[i]
...
local function parse_blocks(blocks)
  ...
  for i=0,11 do
    ...
    -- VLP16
    for i=0, 15 do
      tinsert(dA, rA[i].distance * 2) -- millimeters
      tinsert(iA, rA[i].intensity)
      tinsert(dB, rB[i].distance * 2) -- millimeters
      tinsert(iB, rB[i].intensity)
    end
